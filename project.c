...
void run_custom_timer() {
    ...
    printf("휴식 시간(분)을 입력하세요 (0 입력 시 휴식 없음): "); // 안내 문구 수정
    scanf("%d", &break_min);

    printf("반복할 사이클 수(회)를 입력하세요: ");
    scanf("%d", &cycles);

    // [수정된 부분 1] 유효성 검사 조건 변경
    // break_min <= 0 에서 break_min < 0 으로 변경 (0분 허용)
    if (focus_min <= 0 || break_min < 0 || cycles <= 0) {
        printf(" 집중 시간과 사이클 수는 1 이상, 휴식 시간은 0분 이상이어야 합니다. 설정을 다시 해주세요.\n");
        // 입력 버퍼 비우기 (오류 방지)
        while (getchar() != '\n');
        return;
    }
    ...
    for (int i = 1; i <= cycles; i++) {
        ...
        // [수정된 부분 2] 휴식 시간이 0보다 클 때만 휴식 타이머 실행
        // 0분이면 휴식 타이머를 건너뜀
        if (i < cycles && break_min > 0) {
